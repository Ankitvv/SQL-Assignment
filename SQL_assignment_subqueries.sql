create database SubQueries;

use SubQueries;
																
create table Employee(emp_id int, emp_name varchar(40), department_id varchar(40), salary int)

insert into employee(emp_id, emp_name, department_id, salary)
values
(101, 'Abhishek', 'D01', 62000),
(102, 'Shubham', 'D01', 58000),
(103, 'Priya', 'D02', 67000),
(104, 'Rohit', 'D02', 64000),
(105, 'Neha', 'D03', 72000),
(106, 'Aman', 'D03', 55000),
(107, 'Ravi', 'D04', 60000),
(108, 'Sneha', 'D04', 75000),
(109, 'Kiran', 'D05', 70000),
(110, 'Tanuja', 'D05', 65000);

select * from Employee;

create table Department(department_id varchar(50),
 department_name varchar(50), location varchar(50))

insert into Department(department_id ,
 department_name , location)
 
 values
 ('D01', 'Sales', 'Mumbai'),
 ('D02', 'Marketing', 'Delhi'),
 ('D03', 'Finance', 'Pune'),
 ('D04', 'HR', 'Bengaluru'),
 ('D05', 'IT', 'Hyderabad');
 
 select * from Department;
 
 create table Sales(sale_id int, emp_id int, sale_amount int, 
sale_date date)

insert into Sales(sale_id, emp_id, sale_amount, sale_date)
values
(201, 101, 4500 ,'2025-01-05'),
(202, 102, 7800, '2025-01-10'),
(203, 103, 6700 ,'2025-01-14'),
(204, 104, 12000, '2025-01-20'),
(205, 105, 9800, '2025-02-02'),
(206, 106, 10500, '2025-02-05'),
(207, 107, 3200, '2025-02-09'),
(208, 108, 5100, '2025-02-15'),
(209, 109, 3900, '2025-02-20'),
(210, 110, 7200, '2025-03-01');
    
    select * from sales;
    
    

Basic Level Questions

1. Retrieve the names of employees who earn more 
than the average salary of all employees.


	select emp_name
	  from Employee
	  where salary > 
	  (select avg(salary) from Employee);
      
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      
2.  Find the employees who belong to the department with 
the highest average salary.

select e.emp_name, e.salary, d.department_name from Employee as e 
join Department as d on d.department_id = e.department_id
where e.department_id =

(select distinct department_id from Employee
group by department_id
order by avg(salary) desc
limit 1);

//////////////////////////////////////////////////////////////////////////////////////////////////////

3. List all employees who have made at least one sale.

select e.emp_name from Employee as e 
join Sales as s on e.emp_id = s.emp_id
where s.sale_amount is not null;

/////////////////////////////////////////////////////////////////////////

4. Find the employee with the highest sale amount.

select e.emp_name from Employee as e
join sales as s on e.emp_id = s.emp_id
order by s.sale_amount desc limit 1;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

5.  Retrieve the names of employees whose salaries are higher
 than Shubham’s salary.
 
 select emp_name from Employee
 where salary > 
 (select salary from Employee
 where Emp_name = 'Shubham');
 
 Intermediate level
 
 Q-1.
 Find employees who work in the same department as Abhishek.
 
 select emp_name  as employees from employee
 where department_id =
 (select department_id from employee
 where emp_name = 'Abhishek')
 and emp_name <> 'Abhishek';
 
  
 Q-2 
 List departments that have at least one 
 employee earning more than ₹60,000.
	 
	 select distinct d.department_name
	 from department as d 
	 join employee as e on 
	 e.department_id=d.department_id
	 where e.salary > 60000;
     
     
///////////////////////////////
Q-3.
Find the department name of the employee
 who made the highest sale.
 
 select d.department_name as department from department as d 
 join employee as e on e.department_id=d.department_id
  where e.emp_id IN 
 (select e.emp_id from employee as e join sales as s on 
 e.emp_id=s.emp_id 
 where s.sale_amount IN
(select max(sale_amount) from sales))

 
Q-4.
 Retrieve employees who have made sales greater than 
 the average sale amount.
 
 Select e.emp_name from employee as e 
 join sales as s on e.emp_id=s.emp_id
 where s.sale_amount > 
 (select avg(s.sale_amount) from sales as s);
 
 
 ///////////////////////////////////////////////////////////////////////////////////
 
 Q-5.
 Find the total sales made by employees who earn more 
 than the average salary.
 
 SELECT sum(s.sale_amount) as total_sales 
 from sales as s join employee as e on e.emp_id = s.emp_id
 where e.emp_name in 
(select e.emp_name from employee as e 
where e.salary >
(select avg(e.salary) from employee as e));

////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
Advanced Level
--------------------------------------------

Q-1. 
Find employees who have not made any sales.

Select e.emp_name from employee as e left join sales as s on 
e.emp_id=s.emp_id
where s.emp_id is null

Q-2.
List departments where the average salary is above ₹55,000.

select d.department_name from department as d
join employee as e on e.department_id = d.department_id
group by e.department_id, d.department_name
having avg(e.salary)> 55000;

select e.department_id 
from employee as e 
(select avg(e.salary) from employee as e 
group by e.department_id) > 55000;

	Q-3. 
	Retrieve department names where the total
	 sales exceed ₹10,000.
	 
	 select d.department_name from department as d
	 join employee as e on e.department_id = d.department_id
	 join sales as s 
	 on s.emp_id= e.emp_id
group by d.department_name 
having sum(s.sale_amount) > 10000;

//////////////////////////////////////////////////////////////////////////////
Q-4.
Find the employee who has made the second-highest sale.

SELECT e.emp_id, e.emp_name from employee as e 
join sales as s on s.emp_id = e.emp_id
order by s.sale_amount
limit 1 offset 1;

Q-5.
Retrieve the names of employees whose salary is greater than the highest sale amount recorded.

Select e.emp_name from employee as e 
join sales as s on e.emp_id = s.emp_id
where e.salary > 
(select max(s.sale_amount) from sales as s);






